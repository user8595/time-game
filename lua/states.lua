...
function gameLoop(dt)
  ...
  if isFail then
    ...
    if mode == 1 then
      if pf > hiPerf.normal then
        saveData("score.json",
          { normal = pf, random = hiPerf.random, comboNormal = hiPerf.comboNormal, comboRandom = hiPerf.comboRandom })
      end

      if maxCombo > hiPerf.comboNormal then
        saveData("score.json",
          { normal = hiPerf.normal, random = hiPerf.random, comboNormal = maxCombo, comboRandom = hiPerf.comboRandom })
      end
      loadScore()
    end
    ...
    if mode == 2 then
      if pf > hiPerf.random then
        saveData("score.json",
          { normal = hiPerf.normal, random = pf, comboNormal = hiPerf.comboNormal, comboRandom = hiPerf.comboRandom })
      end

      if maxCombo > hiPerf.comboRandom then
        saveData("score.json",
          { normal = hiPerf.normal, random = hiPerf.random, comboNormal = hiPerf.comboNormal, comboRandom = maxCombo })
      end
      loadScore()
    end
  end
  ...
  if shakeTime > 0 and isShake then
    tX, tY = (oWidth - gWidth) / 2 + love.math.random(2.25, -2.25),
        (oHeight - gHeight) / 2 + 23 + love.math.random(2.25, -2.25)
  end

  if shakeTime > 0 and isShakeHit and speed > 3.5 then
    tX, tY = (oWidth - gWidth) / 2 + love.math.random(1.25, -1.25),
        (oHeight - gHeight) / 2 + 23 + love.math.random(1.25, -1.25)
  end

  if shakeTime > 0 and isShakeHit and speed > 4.25 then
    tX, tY = (oWidth - gWidth) / 2 + love.math.random(1.65, -1.65),
        (oHeight - gHeight) / 2 + 23 + love.math.random(1.65, -1.65)
  end
